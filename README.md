...
3. [De zéro à WordPress — installation complète](#3-de-zéro-à-wordpress--installation-complète)
   - [Étape 1 — Cloner et lancer docker-base](#étape-1--cloner-et-lancer-docker-base)
   - [Étape 2 — Cloner WordPressBuilder](#étape-2--cloner-wordpressbuilder)
...
### Dépendance obligatoire : docker-base

Ce projet nécessite que **[docker-base](../docker-base)** soit installé et en cours d'exécution. Ce projet distinct fournit :

- Le reverse proxy **Traefik** (ports 80 et 443)
- Le réseau Docker partagé `traefik-net`
- Les certificats TLS locaux (`*.local`) via mkcert

Sans `docker-base` lancé, WordPressBuilder ne peut pas démarrer (le réseau `traefik-net` n'existe pas).

Chemin attendu : `~/project/docker-base`
...
```
                    +----------------------------------+
  Navigateur        |      docker-base / Traefik       |  ports 80 / 443
...
Chaque projet WordPressBuilder utilise deux réseaux Docker :

- **`traefik-net`** — réseau externe créé par docker-base. WordPress, phpMyAdmin et Mailpit y sont connectés pour que Traefik puisse les atteindre et router le trafic HTTPS entrant.
...
### Étape 1 — Cloner et lancer docker-base

Si ce n'est pas déjà fait, installer et démarrer docker-base. C'est l'opération à faire **une seule fois** — docker-base reste ensuite actif pour tous les projets.

```bash
git clone <url-docker-base> ~/project/docker-base
cd ~/project/docker-base
cp .env.example .env
...
> Pour que Chrome sur Windows fasse confiance aux certificats, le CA mkcert doit également être importé dans le magasin de certificats Windows. Voir [docker-base/README.md](../docker-base/README.md), section "WSL2 notes".
...
### Étape 2 — Cloner WordPressBuilder

```bash
git clone <url-du-repo> ~/project/WordPressBuilder
cd ~/project/WordPressBuilder
```

Pour utiliser ce projet comme base pour un site nommé `monsite` :

```bash
cp -r ~/project/WordPressBuilder ~/project/monsite
cd ~/project/monsite
```
...
TRAEFIK_NETWORK=traefik-net   # Doit correspondre à docker-base/.env
...
```bash
# Vérifier que le réseau traefik-net existe (docker-base doit tourner)
make check-network
...
| `TRAEFIK_NETWORK` | `traefik-net` | Nom du réseau Docker partagé avec Traefik. Doit correspondre à `TRAEFIK_NETWORK` dans `docker-base/.env`. |
...
Traefik tourne dans docker-base et est partagé entre tous les projets. Un second site n'a pas besoin de relancer docker-base.

```bash
cp -r ~/project/WordPressBuilder ~/project/autresite
...
| `https://traefik.local` | Traefik Dashboard | Vue des routeurs actifs (fourni par docker-base) |
...
|----------|-------------|
| `make start` | Démarre la stack WordPress (docker-base doit tourner) |
...
```bash
make check-network
# Si erreur : démarrer docker-base
cd ~/project/docker-base && make up
```

### Le site affiche une erreur SSL / certificat non reconnu

Les certificats sont gérés par docker-base. Vérifier :

1. Que `make certs` a été exécuté dans docker-base
...
4. Que docker-base tourne : `cd ~/project/docker-base && make ps`
...
cd ~/project/docker-base && docker compose restart traefik
```
